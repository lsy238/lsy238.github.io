(function(window, undefined) {
        ...
        removeAttr: function(elem, value) {
            ...
            if (attrNames && elem.nodeType === 1) {
                while ((name = attrNames[i++])) {
                    ...
                    if (rboolean.test(name)) { if (!getSetAttribute && ruseDefault.test(name)) { elem[jQuery.camelCase("default-" + name)] = elem[propName] = false } else { elem[propName] = false } } else { jQuery.attr(elem, name, "") }
    ...
    if (!getSetAttribute) {
        nodeHook = jQuery.valHooks.button = {
            get: function(elem, name) { var ret = elem.getAttributeNode(name); return ret && (name === "id" || name === "name" || name === "coords" ? ret.value !== "" : ret.specified) ? ret.value : undefined },
    ...
    (function(window, undefined) {
        ...
        setDocument = Sizzle.setDocument = function(node) {
            ...
            sortOrder = docElem.compareDocumentPosition ? function(a, b) { var compare; if (a === b) { hasDuplicate = true; return 0 } if ((compare = b.compareDocumentPosition && a.compareDocumentPosition && a.compareDocumentPosition(b))) { if (compare & 1 || a.parentNode && a.parentNode.nodeType === 11) { if (a === doc || contains(preferredDoc, a)) { return -1 } if (b === doc || contains(preferredDoc, b)) { return 1 } return 0 } return compare & 4 ? -1 : 1 } return a.compareDocumentPosition ? -1 : 1 } : function(a, b) {
                var cur, i = 0,
                    ...
                    ap = [a],
                    bp = [b];
                if (a === b) { hasDuplicate = true; return 0 } else { if (!aup || !bup) { return a === doc ? -1 : b === doc ? 1 : aup ? -1 : bup ? 1 : 0 } else { if (aup === bup) { return siblingCheck(a, b) } } }
                cur = a;
        ...
        Sizzle.matchesSelector = function(elem, expr) {
            ...
            if (support.matchesSelector && !documentIsXML && (!rbuggyMatches || !rbuggyMatches.test(expr)) && !rbuggyQSA.test(expr)) { try { var ret = matches.call(elem, expr); if (ret || support.disconnectedMatch || elem.document && elem.document.nodeType !== 11) { return ret } } catch (e) {} }
        ...
        function matcherFromTokens(tokens) {
            ...
            for (; i < len; i++) {
                if ((matcher = Expr.relative[tokens[i].type])) { matchers = [addCombinator(elementMatcher(matchers), matcher)] } else {
                    ...
                    if (matcher[expando]) { j = ++i; for (; j < len; j++) { if (Expr.relative[tokens[j].type]) { break } } return setMatcher(i > 1 && elementMatcher(matchers), i > 1 && toSelector(tokens.slice(0, i - 1)).replace(rtrim, "$1"), matcher, i < j && matcherFromTokens(tokens.slice(i, j)), j < len && matcherFromTokens((tokens = tokens.slice(j))), j < len && toSelector(tokens)) }
...
$(document).ready(function() {
    ...
    var aidParam = window.location.href.toString().split("aid=");
    if (aidParam.length == 2) {
        var aid = aidParam[1];
        console.log(aid);
        $.get("https://pcw-api.iqiyi.com/albums/album/avlistinfo?aid=" + aid + "&page=0&size=10",
            function(data, status) {
                console.log(data);
                data = data.data;
                var episodeList = data.epsodelist;
                var currV = episodeList[0];
                $("#v-title").html(currV.shortTitle);
                $("#v-description").html(currV.description);
                var episodelistHtml = "";
                for (var i = 0; i < data.total; i++) {
                    episodelistHtml += "<button onlick='changeI(" + i + ")' class='btn btn-success' data-i='" + i + "'>" + (i + 1) + "</button>";
                }
                $("#v-episodelist").html(episodelistHtml);
                $("#v-episodelist").on('click', '[data-i]', function() {

                    var i = $(this).attr('data-i');
                    $("#v-title").html(episodeList[i].shortTitle);
                    $("#v-description").html(episodeList[i].description);
                    console.log(i);
                    var parsei = $("#parse-interface").val();
                    var url = iObject[parsei] + episodeList[i].playUrl;
                    $("iframe").attr("src", url)
                    $("#v-url").val(episodeList[i].playUrl);
                    $(this).attr("class", "btn btn-warning");
                    $(this).siblings().attr("class", "btn btn-success")
                });
            });
    }
